// 显式注解函数参数：ts 能推导但是多数情况下无法推导出参数类型
...
// call,apply,bind
// apply()与 call()跟 js 用法一致，第一个参数绑定 this，第二个参数作为参数传入被调用的函数中；call 按顺序应用参数而非数组
// bind() 并不调用函数，而返回一个新函数，通过（），.call 或者 .apply 调用

// 注解 this 的类型
// this 的值却月与调用函数的方式，而不受声明方式的干扰


// ！！！！！Ts：函数如果使用 this，要在函数的第一个参数中声明 this 的类型（放在其他参数之前）-确保 this 时期待的类型
...
// 可迭代对象--有 Symbol.iterator 属性的对象，而且该属性的值为一个函数，返回一个迭代器
...
// 重载函数---有多个调用签名的函数
// js 是一门动态语言，需要多种方式调用一个函数的方法，甚至有时输出的类型取决于输入的类型
...
// 绑定泛型的时间
// 在调用签名中声明（位于签名的开始圆括号前面），ts 将在调用这个类型的函数时为 T 绑定具体类型
type Filter={
    <T>(array:T[],f:(item :T[])=>void):T[]
}

// 把 T 的作用域限定在类型别名中，ts 则要求使用其时显式绑定类型
